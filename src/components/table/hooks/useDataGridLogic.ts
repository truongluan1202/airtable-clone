...
export function useDataGridLogic(
    ...
    data.forEach((row) => {
      columns.forEach((column) => {
        ...
        if (cellValue !== null && cellValue !== undefined) {
          ...
          if (cellValueStr.includes(query)) {
            highlighted.add(`${row.id}-${column.id}`);
          }
        }
      });
    });
  ...
  // Check if a cell matches the search query - now just a simple lookup
  const isCellHighlighted = useCallback(
    (rowId: string, columnId: string, _cellValue: string) => {
      return highlightedCells.has(`${rowId}-${columnId}`);
    },
    [highlightedCells],
  );
